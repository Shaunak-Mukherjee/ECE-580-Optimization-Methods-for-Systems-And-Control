...
for i= [1,2]
    ...
    while norm(grad(x_iter(1),x_iter(2))) > tol
        ...
        % Correction factor              
        Hk_new = Hk + (del_x * del_x')/(del_x'* del_grad) - (((Hk * del_grad)) * (Hk * del_grad)')/(del_grad' * Hk * del_grad);
        
        % if norm((del_grad'*(del_x - Hk * del_grad))) <1e-8 % Tolerance added unecessary iterations
        %      break
        %  end
        
         % Reinitilize the directional vector every 2 iterations 
